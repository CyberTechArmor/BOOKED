...
export default function AvailabilityPage(): JSX.Element {
  ...
  // Derive selected schedule from store to keep it in sync
  ...
  }, [formData, editingSchedule, updateSchedule, createSchedule]);
  ...
  }, [deleteSchedule, selectedScheduleId, schedules]);

  const handleAddWindow = useCallback(async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedScheduleId) return;

    try {
      await addWindow(selectedScheduleId, windowForm);
  ...
  }, [selectedScheduleId, windowForm, addWindow]);
  ...
  const handleDeleteWindow = useCallback(async (windowId: string) => {
    ...
    try {
      await deleteWindow(selectedScheduleId, windowId);
    } catch (err) {
      alert(err instanceof Error ? err.message : 'Failed to delete time window');
    }
  }, [selectedScheduleId, deleteWindow]);

  const getWindowsForDay = useCallback((dayOfWeek: number) => {
                                ...
                                {windows.map((window) => (
                                  ...
                                  >
                                    ...
                                    <button
                                      ...
                                      aria-label={`Delete ${day} ${window.startTime} - ${window.endTime}`}
                                    >
